...
public class add_StudentIDGUI extends Form {

	public add_StudentIDGUI() {
		super("Add Student ID");
		addGUIcomponent();
	...
	private void addGUIcomponent() {
		...
		// add student button
		JButton addStudent = new JButton("Add Student ID");
		addStudent.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED));
		addStudent.setFocusPainted(false);
		addStudent.setForeground(new Color(34, 240, 34));
		addStudent.setFont(new Font("Dialog", Font.BOLD, 18));
		addStudent.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));// makes hand cursor when hovering
		addStudent.setBackground(new Color(16, 16, 16));
		addStudent.setBounds(250, 400, 175, 40);
		...
		addStudent.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String EMPLID = MyJDBC.generateID();

				// Validate User Input
				if (validateUserInput(First, Last, DOB, EMPLID)) {
					// Register Student to the database
					if (MyJDBC.register(First, Middle, Last, DOB, Year, Email, Address, Phone, EMPLID)) {
						// Disposes add_Student GUI
						add_StudentIDGUI.this.dispose();
						// Takes back to main page
						ID_MainGUI idMainGUI = new ID_MainGUI();
						idMainGUI.setVisible(true);
						// Creates a result dialog
						JOptionPane.showMessageDialog(idMainGUI, "Student Successfully Added");
					} else {
						...
						ID_MainGUI idMainGUI = new ID_MainGUI();
						idMainGUI.setVisible(true);
						JOptionPane.showMessageDialog(add_StudentIDGUI.this, "Failed: Student ID Already Exists");
					}
				} else {
					...
					ID_MainGUI idMainGUI = new ID_MainGUI();
					idMainGUI.setVisible(true);
					JOptionPane.showMessageDialog(add_StudentIDGUI.this, "Invalid Input");
				}
			}
